...
public class Staff extends JFrame implements ActionListener{
    ...
    public void AddStaff(){
        ...
        staff=new StaffDetails(id,name,age,gender,job,salary,phone);
        //exception handling and file handling for database
        try{
            FileWriter writer=new FileWriter("src/Hotel_Management_System/Person/Details/TextFiles/StaffDetails.txt",true);
            writer.write(staff.getId()+","+staff.getName()+","+staff.getAge()+","+staff.getGender()+
                    ","+staff.getJob()+","+staff.getSalary()+","+staff.getPhone()+"\n");
            writer.close();
    ...
    public String generateId() {
        int lastId = 2000; // Default starting ID
        try (BufferedReader reader = new BufferedReader(new FileReader("src/Hotel_Management_System/Person/Details/TextFiles/StaffDetails.txt"))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split(",");
                if (parts.length > 0) {
